...
**This is a study of Russian, not of Ruthenian.** It is one of the source
languages the lexicon draws cognates from (`DIRECTION.md`, "The lexicon is
multi-source"), and it is described here so that reconstruction has evidence to
work from. Nothing in this file is normative for Ruthenian; the language is
specified in [`../RUTHENIAN.md`](../RUTHENIAN.md), which outranks this document
on every question about what Ruthenian does.

Read it for what Russian *has* — and, just as often, for what Russian has
**lost**, since that is why the lexicon needs Ukrainian, Polish and OCS as well.
Russian merged yat, levelled the second palatalization to 0 %, retains 40 relic
vocatives, and has no dual, no ablative and no aorist. Every one of those is a
category Ruthenian restores from elsewhere.

Two purposes: record what the standard descriptive tradition says, and inventory
every irregularity actually measured, so that decisions about which of them
Ruthenian keeps are made against evidence rather than intuition. Per
`INVARIANTS.md` I7, claims here are **measured** — they are claims about a source
language, and that is the standard those must meet.
