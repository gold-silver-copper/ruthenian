...
pub enum Origin {
    /// The rule engine produced it under `Variant::standard()`. This is the
    /// ordinary case — the language is specified, so its forms are derived.
    RuleDerived,
    /// Stored in the tables because no rule predicts it.
    Stored,
    /// Produced under an enabled optional feature (`../RUTHENIAN.md` §13).
    Variant(RuleId),
}
...
rule, stored because no rule predicts it, or produced under an optional variant.
...
7. `Origin::Variant` always names a `RuleId` that exists in the core registry.
