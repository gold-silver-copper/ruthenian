...
- `--json` on **every** subcommand. Same information as the human output, no
  more, no less.
- `--variant` on every generating subcommand, naming optional features to
  enable; `standard` is the default and is the language as specified.
- `--show-derivation` prints the `RuleId` and trace for any form, so a user can
  see which rule produced it.
...
2. `--json` output for a given invocation is a lossless encoding of the human
   output.
3. Every generating subcommand honours `--variant`, and the active variant
...
| `variant_echoed` | Inv. 3 | Accept `--variant` and ignore it; output under standard and a feature-enabled variant is byte-identical for a rule-affected lemma | required | ms | crate |
...
| `colour_carries_no_information` | §10 — piped output loses nothing but styling | Convey a gap or a variant origin by colour alone; the piped transcript no longer distinguishes it | required | ms | crate |
...
- Every subcommand in §2 implemented with `--json`, `--variant` and
