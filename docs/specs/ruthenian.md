...
pub fn form(key: &EntryKey, slot: Slot, variant: &Variant) -> Option<Form>;

/// The whole paradigm, in a defined order.
pub fn paradigm(key: &EntryKey, variant: &Variant) -> Option<Paradigm>;
...
4. `None` means "no such form exists" — a genuine paradigm gap under the active
   variant. It never means "not in the tables".
5. `variant` is the only thing that changes output for a fixed (key, slot). No
...
| `variant_is_the_only_variable` | Inv. 5 | Read a `static mut` or env var in the lookup path | required | ms | crate |
...
- ~~Which variant is the default~~ — **closed**: `Variant::standard()`, the
  language exactly as `docs/RUTHENIAN.md` specifies it. Optional features are
  opt-in, so default output is checkable against the specification. There is no
  preset that reproduces a source language: the regularizations are what the
  language is, not a mode it can be taken out of.
- **`resolve` does not rank.** It returns every candidate in a deterministic
  ...
  getting it wrong.
- **`Variant` is a preset plus adjustment**: `standard()` plus `.with(rule)` /
  `.without(rule)`. The common case is one call, and the evaluator can isolate a
  single optional feature — which is what Phase 6 needs in order to report them
  one at a time. `VariantId` in the eval summary must therefore identify an
  arbitrary combination, not just the preset.
...
One, and it is owned by the language rather than by this crate: **which optional
features `Variant` offers**, governed by `docs/RUTHENIAN.md` §13. A feature ships
enabled only once the spec settles it; until then each is off in the standard
variant and reachable through `Variant::standard().with(rule)`.
