...
/// Why a form looks the way it does: the rules that fired, in order. Never
/// empty.
///
/// Provenance, not configuration: a trace *describes* an answer rather than
/// selecting one. There is no registry behind it and nothing to switch on or
/// off, because none of these rules is optional.
pub struct Trace { .. }
```

Note what is absent: there is no `Policy`, no `Variant`, no feature flag, and no
parameter that changes an answer for a fixed input. See §7.
...
In: a stem, a class, a slot. Out: `Option<Prediction>`. No files, no lookups, no
...
- The trace vocabulary: the rule names a `Prediction` can carry.
...
Generation is a function: `(stem, class, slot) -> Option<Prediction>`. No policy,
no variant, no feature flags, nothing that changes an answer for a fixed input.
...
of sixteen, fixed stress, derived aspect, no indeclinables — are not options.
They are what the language *is* (`../RUTHENIAN.md` §1). The thing on the other
side of such a switch would be Russian, and this crate does not generate Russian.

**Neither are the open questions.** An earlier version of this spec proposed a
`Variant` type switching `../RUTHENIAN.md` §13's undecided items — the ablative
plural, clitic pronouns, the middle voice. That was wrong in two ways.

It was wrong *mechanically*: no such feature may ship enabled while the spec
still calls the question open, so every rule in the registry would be permanently
disabled, and every affected rule in the engine would carry a dead branch waiting
on a decision nobody had made. Law 5 forbids exactly that — state that duplicates
something not yet decided, whose unused arm becomes the bug.

It was wrong *categorically*: §13's entries are **language-design questions
awaiting an answer**, not options a caller chooses between. Modelling them as
runtime configuration presupposes that Ruthenian ships as several simultaneous
dialects. Nothing in the brief calls for that, and the cost of pretending
otherwise is paid in every paradigm.

So an open question **blocks the paradigm it affects** rather than
parameterizing it. When §13 settles one, the specification changes, this crate
changes with it, and the changelog enumerates the affected slots. The spec moves
first; the code follows.

The supine is the worked example of that process running correctly: §7.10a
specifies it completely, so it is simply implemented, and `VerbSlot::Supine` is
unconditional.

> **Spec inconsistency, unresolved here.** §13 item 7 still lists the supine as
> "not yet specified", while §7.10a specifies it completely — infinitive `-tj`
> against supine `-t`, governing the genitive (`idu lovit zvjerjej`) — and
> §"Written in this revision" claims it as newly written. §13 item 7 appears to
> be the stale entry, and is read that way here. If that is wrong, the fix
> belongs in the spec.

**What survives is provenance, which is not configuration.** Every `Prediction`
carries a `Trace` naming the rules that fired, in order. A trace describes an
answer; a policy selects one. The first is owed to every caller — the evaluator
attributes a mismatch through it and the CLI's `--show-derivation` explains a
form with it — and it is as useful under one fixed grammar as under a
configurable one.
...
4. Output is a function of (stem, class, slot) alone — no hidden state, no
   ambient configuration, and nothing to configure in the first place.
5. Rules are total on their declared input domain: any `Ruthenian` stem is
   accepted; garbage in yields a form, not a panic.
6. The public API exposes no configuration type. A parameter that changes output
   for a fixed (stem, class, slot) is a design error, not a feature.
7. Every output string is valid Ruthenian per `ruthenian-orthography`.
8. **The engine reproduces `../RUTHENIAN.md`.** Every form the spec states, for
   every paradigm it tabulates, is produced exactly. This is the invariant the
   whole crate exists to satisfy.
...
| `no_configuration_axis` | Inv. 6 | Add `pub struct Variant` or a `policy` parameter; the check on the public API surface fails | required | ms | crate |
| `generation_is_pure` | Inv. 4 | Read an environment variable inside a rule | required | ms | crate |
| `output_is_valid_ruthenian` | Inv. 7 | Emit a raw `ъ` or a stray uppercase mid-word | required | seconds | crate |
| `noun_paradigms_conform` | **Inv. 8** — every cell the corpus records is reproduced | Change any ending; the failing row names the paradigm and cell it contradicts | required | seconds | crate |
| `spec_corpus_is_current` | The committed corpus still matches `../RUTHENIAN.md` | Amend a form in the spec without regenerating; the currency check diffs | required | seconds | crate |
...
Twelve guards. Two are worth their cost specifically because of what this crate
is:

The conformance pair is the important part, and the split between them is
deliberate.

`../RUTHENIAN.md`'s paradigm tables are **extracted once into a committed
artifact** — `tests/paradigms/spec_nouns.tsv` — and `noun_paradigms_conform`
asserts the engine against that file. `spec_corpus_is_current` re-extracts and
fails with a diff if the specification has moved, so amending the spec shows up
as a corpus diff in review.

Parsing the markdown *inside* the assertion is the tempting shortcut and it does
not work. It was tried: the heading match found `noczj` in §3.2's declension
summary rather than §3.6's paradigm and silently compared `dom`'s forms against
`noczj`, reporting a clean parse throughout. Prose formatting then becomes the
test's interface — a renamed heading or a reworded cross-reference makes the
check quietly cover less — and it needs further guards to protect it from passing
vacuously, which is the signal that the design is wrong.

A committed artifact fixes all of it: the assertion reads stable data, a human
can read what the test believes, and drift is a reviewable diff rather than a
silent narrowing. This is what `interslavic-phrase` actually does — its Steen
corpus is a committed TSV, mined once, not a live parse of Steen's HTML.
...
- Every paradigm table in `../RUTHENIAN.md` §§3–7 is extracted into the committed
  corpus and reproduced exactly. A cell the spec does not state is reported as a
  spec gap, not filled by inference.
- No configuration type in the public API, enforced by `no_configuration_axis`.
- Twelve guards present, each demonstrated to fail under its witness.
...
- **`../RUTHENIAN.md` §13's open questions** — the ablative plural, clitic
  pronouns, the middle voice. Each changes a paradigm this crate implements, so
  each blocks the affected paradigm rather than parameterizing it. Nothing is
  modelled until the specification answers. (The question of whether they should
  instead be runtime options is closed: no. See §7.)
...
- ~~Pronoun and numeral classes~~ — **closed: own `Slot` variants.** See §3.
  ...
  an open §13 question, and until it is answered the language has one series.)
