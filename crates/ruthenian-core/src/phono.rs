...
/// Which palatalization an ending triggers on a preceding velar.
///
/// Ruthenian keeps all three (`RUTHENIAN.md` §2.4). Russian levelled the second
/// away entirely (0 %); Ukrainian keeps it at 99 % and OCS at 66 %. It is not
/// decoration: it is what distinguishes the locative `druzi` from the vocative
/// `druzzje` in the consonant, while `-i` against `-je` distinguishes them in
/// the vowel. A morphophonology module ported from a Russian implementation will
/// not have it, and every velar stem is then silently wrong in two cells.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub enum Palatal {
    /// No velar change.
    None,
    /// Before front vowels and `j` — the vocative `-je`, the present stem, the
    /// comparative. `k`→`cz`, `g`→`zz`, `h`→`sz`.
    First,
...
/// Apply a palatalization to a stem's final velar. Non-velar stems are returned
/// unchanged, which is why this is safe to call unconditionally.
///
/// ```
/// use ruthenian_core::phono::{palatalize, Palatal};
